...
void LRU_Local(pair<int,int> p, int i)
{
    ...
    if(!is_Present1[p.second][p.first])
    {
        ...
        if(!f){
            ...
            { 
                if (value1[Memory[l].first][Memory[l].second] < lru) 
                { 
                    lru = value1[Memory[l].first][Memory[l].second]; 
                    val = l; 
                } 
            }
...
int h=0;
    ...
    cout<<endl;
    cout<<"For local page replacement lru: "<<endl<<endl;
