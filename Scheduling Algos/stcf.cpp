...
void STCF(vector<pair<ll,ll>> proc_arr ,ll proc_burs[] , int n , ll turn_around_time[] , ll response_time[] , ll waiting_time[]) 
{ 
    ll rem_time[n];
    for(ll i =0;i<n;i++)
    {
...
while(k<n)
    ...
    ll j=0;
    ll min = INT_MAX;
    while(proc_arr[j].first<=t && min>rem_time[j] )
    {
...
{ 
 ...
 cout<<"Enter Quantum value : " ;
    ...
    // }
    STCF(proc_arr , proc_burs, n, turn_around_time ,response_time, waiting_time); 
    return 0; 
} 